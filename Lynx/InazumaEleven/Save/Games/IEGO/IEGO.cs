...
namespace Lynx.InazumaEleven.Save.Games.IEGO
{
    public class GO : IGame
    {
        ...
        private Dictionary<int, Item> GetInventory()
        {
            ...
            Dictionary<int, Item> itemBlockGroup1 = Data.ReadMultipleStruct<IEGOHelper.ItemBlockGroup1>(256).Where(x => Items.ContainsKey((int)x.ID) && x.ID != 0x00).ToDictionary(x => x.Index, y => new Item(y.ID, Items[(int)y.ID], y.Quantity));

            // Group Item 2
            Data.Seek((uint)helper.ItemBlockGroup2Offset);
            
            Dictionary<int, Item> itemBlockGroup2 = Data.ReadMultipleStruct<IEGOHelper.ItemBlockGroup2>(224).Where(x => Items.ContainsKey((int)x.ID) && x.ID != 0x00).ToDictionary(x => x.Index, y => new Item(y.ID, Items[(int)y.ID], y.Quantity, y.QuantityEquiped));

            // Group Item 3
            Data.Seek((uint)helper.ItemBlockGroup3Offset);
            Dictionary<int, Item> itemBlockGroup3 = Data.ReadMultipleStruct<IEGOHelper.ItemBlockGroup3>(512).Where(x => Items.ContainsKey((int)x.ID) && x.ID != 0x00).ToDictionary(x => x.Index, y => new Item(y.ID, Items[(int)y.ID]));
        ...
        public GO(Stream data, Dictionary<int, InazumaEleven.Save.Logic.Player> players, Dictionary<int, InazumaEleven.Save.Logic.Move> moves, Dictionary<int, InazumaEleven.Save.Logic.Avatar> avatars, bool isJP)
        {
            Players = players;
            Avatars = avatars;
            Moves = moves;
